...
<div class="row justify-content-center">
    <div class="col-lg-10">
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body">
                <form action="{{ route('cliente.perfil.update') }}" method="POST" id="profile-form">
                    @csrf
                    @method('PUT')
...
$(document).on('change', '#foto_perfil', function() {
    if (this.files && this.files[0]) {
        ...
        if (file.size > 12 * 1024 * 1024) {
            showAlert('Error', 'El archivo es demasiado grande. Máximo 12MB.', 'error');
            return;
        }
...
$('#profile-form').on('submit', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            setTimeout(() => {
                window.location.href = '{{ route("cliente.perfil.show") }}';
            }, 1000);
        },
        error: function(xhr) {
...
function removeProfilePhoto() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("cliente.perfil.remove-foto") }}',
                method: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
